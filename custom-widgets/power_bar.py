...
class _Bar(QtWidgets.QWidget):
    ...
    def paintEvent(self, e):
        ...
        rect = QtCore.QRect(0, 0, int(painter.device().width()), int(painter.device().height()))
        painter.fillRect(rect, brush)

        # Get current state.
        parent = self.parent()
        vmin, vmax = parent.minimum(), parent.maximum()
        if type(self.value) != int and self.value != None:
    ...
    #     return super().event(event)  # Pass unhandled events to the parent
